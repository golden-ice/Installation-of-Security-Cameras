...
class Traffic(object):
    ...
    def printEPrime(self):
        edges = "E {"
        for i in range(len(self.E)):
            if i != len(self.E) - 1:
                edges = edges + "<" + str(self.E[i][0]) +"," + str(self.E[i][1]) + ">,"
            else:
                edges = edges + "<" + str(self.E[i][0]) +"," + str(self.E[i][1]) + ">"
        edges = edges + "}\n"
...
def myParser(input):
    ...
    try:
        ...
        print('Error: a1 parse input error > ' + str(e), file = sys.stderr)
    ...
    if cmd == "gg":
        ...
        for i in range(0, len(testC)):
            ...
            try:
                ...
                print('Error: cannot convert coordinates to int > ' + str(e), file = sys.stderr)
        ...
        raise Exception("wrong command! ..>" + input)
    return cmd, road, coors
        ...
        try:
            ...
            if cmd == "rm":
                try:
                    ...
                    print('Error: rm error > ' + str(e), file = sys.stderr)
            if cmd == "mod":
                try:
                    traffic.mod(road, coors)
                except Exception as e:
                    print('Error: mod error > ' + str(e), file = sys.stderr)
            ...
            if cmd == "gg":
                ...
                # traffic.printV()
                # traffic.printE()
                sys.stdout.write("V " + str(len(traffic.getV())) + "\n")
                sys.stdout.flush()
                traffic.printEPrime()
                sys.stdout.flush()
        except Exception as e:
            print('Error: input to a1 error > ' + str(e), file = sys.stderr)
            sys.exit(1)
