// Compile with c++ find-shortest-paths.cpp -std=c++11 -o find-shortest-paths
#include <iostream>
#include <sstream>
#include <vector>
#include <map>
...
int shortestPathUnweighted(std::map<int, std::vector<int>>& graph, int sv, int ev) {
    ...
    if (it1 == graph.end()) {
        std::cout << "Error: the start vertex is isolated!" << std::endl;
        return -1;
    }
    std::map<int, std::vector<int>>::iterator it2 = graph.find(ev);
    if (it2 == graph.end()) {
        std::cout << "Error: the end vertex is isolated!" << std::endl;
        return -1;
    }
    ...
    if (pathLength[ev] == INT_MAX) {
        std::cout << "Error: there is no path between start vertex and end vertex!" << std::endl;
        return -1;
    } else {
    ...
    while (!std::cin.eof()) {
        ...
        while (!input.eof()) {
            ...
            if (signal == 'V') {
                ...
                input >> numV;
                if (numV <= 1) {
                    std::cout << "Error: number of vertices should be greater than 1" << std::endl;
                    numV = -1;
                }
                ...
                // if 1, means the given edges hasve problems
                int failFlag = 0;

                if (numV == -1) {
                    std::cout << "Error: vertices haven't been given" << std::endl;
                    break;
                }

                if (!givenG.empty()) {
                    std::cout << "Error: Edges have already existed" << std::endl;
                    break;
                }
                ...
                for (const auto& edge : edgeVec) {
                    ...
                    if (edge.first < 1 || edge.first > numV || edge.second < 1 || edge.second > numV) {
                        failFlag = 1;
                        std::cout << "Error: Given edges contains non-exisisting vertex!" << std::endl;
                        givenG.clear();
                        break;
                    } else {
                ...
                */
                ...
                if (start < 1 || start > numV || end < 1 || end > numV) {
                    std::cout << "Error: The start or end vertex does not exist!" << std::endl;
                    break;
                }
                // std::cout << "--Start Processing---" << std::endl;
                int result = 99;
                // calculate the shortest path
                result = shortestPathUnweighted(givenG, start, end);
